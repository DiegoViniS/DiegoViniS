# 👋 Olá, eu sou o Diego Vinicius

Sou um entusiasta de tecnologia e programação, sempre explorando novas ideias e criando projetos próprios.  
Atualmente focado em **JavaScript**, **Python** , **desenvolvimento de jogos** e **Aplicações IA** (Godot e Roblox Studio).

---

## 🚀 Sobre mim

- 🎮 Desenvolvo jogos e protótipos criativos (Godot, Roblox, HTML5)
- 💻 Aprendendo e aplicando **automação com Python**
- 📊 Interessado em **finanças pessoais, dados e produtividade**
- 🌱 Sempre buscando melhorar minhas habilidades técnicas e criativas

---

## 🧠 Tecnologias e Ferramentas

> Algumas linguagens e ferramentas que uso com frequência:

- **Linguagens:** JavaScript, Python, GDScript  
- **Front-end:** HTML5, CSS3  
- **Game Dev:** Roblox Studio  
- **Outros:** Git, VS Code, Excel, OBS Studio  

---

## 📂 Projetos em Destaque

- 🎮 *FazendaViva*: jogo estilo fazenda feito na Godot  
- ⚔️ *Anime Battle Project*: protótipo de RPG browser com tema anime  
- 📈 *Controle Financeiro Pessoal*: automação simples em Python e planilha Excel  

---

## 📫 Contato

- 💼 GitHub: [@KaiM1K](https://github.com/KaiM1K)  
- 🎥 YouTube / Kick: *KaiM1K*  
- ✉️ Email: *(diegovinicius4d@hotmail.com)*

---

⭐ **Se curtir algum projeto, deixe uma estrela!**  
Cada estrela ajuda a fortalecer meu portfólio e minha jornada como desenvolvedor 🚀

